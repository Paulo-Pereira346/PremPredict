...
def predict_outcome(lambda_home, lambda_away, max_goals=10):
    ...
    return max(probs, key=probs.get), scoreline["home"], scoreline["away"]
...
for fold, (train_index, test_index) in enumerate(tscv.split(X)):
    ...
    for i in range(len(Y_home_pred)):
        outcome, _, _ = predict_outcome(Y_home_pred[i], Y_away_pred[i])
...
print("(Poisson) Avg MAE Home:", sum(MAE_home_list_Pois)/len(MAE_home_list_Pois))
print("(Poisson) Avg MAE Away:", sum(MAE_away_list_Pois)/len(MAE_away_list_Pois))
